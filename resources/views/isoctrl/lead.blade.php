...
$(document).on('click', '.show-vcomments-modal', function() {
    ...
    <tr>
        ...
        $zip= "../public/storage/isoctrl/lead/attach/".$afilename[0].".zip";
        $bfile= "../public/storage/isoctrl/lead/attach/".$afilename[0].".b";
        $dxf= "../public/storage/isoctrl/lead/attach/".$afilename[0]."-01.dxf";
        $cii= "../public/storage/isoctrl/lead/attach/".$afilename[0].".cii";

        $pdftie= "../public/storage/isoctrl/lead/attach/".$afilename[0]."-TIE.pdf";
        $pdfspo= "../public/storage/isoctrl/lead/attach/".$afilename[0]."-PROC.pdf";
        $pdfsit= "../public/storage/isoctrl/lead/attach/".$afilename[0]."-INST.pdf";
      ...
      <td>
           ...
           <?php if ($requested[0]->requested!=1 AND $requested[0]->requestedlead!=1){ ?> <!-- Si no tiene solicitud desde diseño puede enviar a siguiente etapa -->
            ...
            <?php if (1==1/* DESHABILITA VALIDACIÓN DE ADJUNTO 1==1 file_exists($pdfcl) AND file_exists($zip) AND $requested[0]->spo==0 AND $requested[0]->sit==0*/) { ?>
            ...
            <?php }elseif ($requested[0]->spo!=0 AND $requested[0]->sit!=0 AND file_exists($pdfspo) AND file_exists($pdfsit)) { ?>

                <a class="comments-lead-modal btn btn-xs btn-success" data-filename ="<?php echo $filename[$i]; ?>" data-requestbydesign ="<?php echo $requested[0]->requested; ?>" data-requestbylead ="<?php echo $requested[0]->requestedlead; ?>" data-toggle="modal" data-target="#commentsfromleadModal">Isoctrl</a>

            <?php }elseif ($requested[0]->spo!=0 AND $requested[0]->sit==0 AND file_exists($pdfspo)) { ?>

                <a class="comments-lead-modal btn btn-xs btn-success" data-filename ="<?php echo $filename[$i]; ?>" data-requestbydesign ="<?php echo $requested[0]->requested; ?>" data-requestbylead ="<?php echo $requested[0]->requestedlead; ?>" data-toggle="modal" data-target="#commentsfromleadModal">Isoctrl</a> 

            <?php }elseif ($requested[0]->sit!=0 AND $requested[0]->spo==0 AND file_exists($pdfsit)) { ?>
            ...
            <?php }else{ ?>
              ...
              <a class="comments-design-modal btn btn-xs btn-info" data-filename ="<?php echo $filename[$i]; ?>" data-toggle="modal" data-target=""><b>MISSING!</b></a>
          ...
          <?php if ($requested[0]->requested!=1 AND $requested[0]->requestedlead!=1){ ?> <!-- Si no tiene solicitud desde diseño puede actualizar --> 
             ...
             <!--  <?php
                      ...
                      //if($zipfile)
                      {
                          ...
                          //while ($zip_entry = zip_read($zipfile))
                          {
                             ...
                             //echo "<p>Nombre del archivo: " . zip_entry_name($zip_entry) . "\n";
                             //echo "<br/>";
   ...
   <center>
        ...
        <button onclick="location.href='{{ url('lead') }}'" type="button" class="btn btn-primary btn-lg" style="width:15%"><b>Issuer</b></button>&nbsp;&nbsp;
