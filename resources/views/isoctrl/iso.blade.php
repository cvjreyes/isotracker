...
<script type="text/javascript">
  ...
  $(document).on('click', '.reject-iso-modal', function() {
            ...
            $('.requestbydesign').val($(this).data('requestbydesign'));
            $('.requestbylead').val($(this).data('requestbylead'));
            $('.issued').val($(this).data('issued')); // PARA LAS EMITIDAS
            $('.to').val($(this).data('to')); // PARA LAS EMITIDAS
...
$(document).on('click', '.show-vcomments-modal', function() {
    ...
    <tr>
        ...
        $zip= "../public/storage/isoctrl/iso/attach/".$afilename[0].".zip";
        $bfile= "../public/storage/isoctrl/iso/attach/".$afilename[0].".b";
        $dxf= "../public/storage/isoctrl/iso/attach/".$afilename[0]."-01.dxf";
        $cii= "../public/storage/isoctrl/iso/attach/".$afilename[0].".cii";

        $pdftie= "../public/storage/isoctrl/iso/attach/".$afilename[0]."-TIE.pdf";
      ...
      <td>
        ...
        <?php if (auth()->user()->hasRole('IsoctrlAdmin')){ ?> 
          ...
          <?php if ($requested[0]->requested!=1 AND $requested[0]->requestedlead!=1){ ?> <!-- Si no tiene solicitud desde diseño o Lead puede enviar a siguiente etapa -->
               ...
               <a class="upload-design-modal btn btn-xs btn-info" data-filename ="<?php echo $filename[$i]; ?>" data-pathfrom="iso"  data-requestbydesign ="<?php echo $requested[0]->requested; ?>" data-requestbylead ="<?php echo $requested[0]->requestedlead; ?>" data-toggle="modal" data-target="#uploadfromdesignModal">Upload File</a>
          ...
          <?php } ?>  
           ...
           <?php }elseif (auth()->user()->hasRole('DesignAdmin')){?>
            ...
            <?php } ?>
              ...
              <?php }else{ // REQUEST "EMITIDA" BY DESIGN?>

                <a class="comments-iso-modal btn btn-xs btn-success" data-filename ="<?php echo $filename[$i]; ?>" data-toggle="modal" data-target=""><b><?php echo $issued[0]->to; ?></b></a>
              
                   <?php if (strpos($requested[0]->to, '(D)') == false) { // SI FUE EMITIDA Y ENVIADA A DISEÑO A NUEVA REVISION, DESAPARECE EL BOTÓN REQUEST POR TENER (D)?>     
    ...
    <!-- BUTTONS FOR SELECT ALL  --> 
      ...
      Date: {!! Form::text('issuedate', $issuedate, array('id' => 'issuedate','style'=>'height:34px;font-size: 14px;font-weight: normal;',required)) !!}
